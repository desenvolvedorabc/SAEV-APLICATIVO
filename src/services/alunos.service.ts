...
export async function getStudentsByProfile(
  search: string,
  page: number,
  limit: number,
  column: string,
  ...
  const resp = await api.get("/student/profile", { params });
  return resp;
}

export function useGetStudentsByProfile(
  search: string,
  page: number,
  limit: number,
  column: string,
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["students-profile", params],
    queryFn: async () => {
      const response = await api.get("/student/profile", { params });
...
export async function getAllStudents() {
  const params = { token };
  return axios.get("/api/student/all", { params });
}
