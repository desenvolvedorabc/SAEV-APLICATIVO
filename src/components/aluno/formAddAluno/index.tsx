...
export function FormAddStudent({munId}) {
  ...
  const validationSchema = yup.object({
    ...
    ALU_EMAIL: yup
      ...
      .nullable(),
    ALU_DT_NASC: yup.string().required("Campo obrigatório"),
    ALU_CEP: yup
      .string()
  ...
  const loadSchoolClass = async (idSchool, idSerie) => {
    const respSchoolClass = await getSchoolClassSerie(idSchool, idSerie, '1');
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (
        response.status === 200 &&
        response.data.ALU_NOME === values.ALU_NOME
      ) {
        setModalStatus(true);
  ...
  return (
    <>
        ...
        >
          <div className="card-box">
            ...
            <InputGroup3Dashed className="" controlId="formBasic">
              <div>
                <AutoCompletePagEscMun changeSchool={handleChangeSchool} school={school} mun={{MUN_ID: munId}} active={"1"}/>
          ...
          <div className="card-box">
            ...
            <InputGroup3Dashed className="" controlId="formBasic">
              ...
              <div>
                <FormControl fullWidth size="small">
                  <InputLabel id="ALU_PCD">Deficiência</InputLabel>
                  <Select
                    labelId="ALU_PCD"
                    id="ALU_PCD"
                    name="ALU_PCD"
                    value={formik.values.ALU_PCD}
                    label="Deficiência"
                    onChange={formik.handleChange}
                    disabled={!school}
                  ...
                  >
                    {listPcd?.map((item) => (
                      <MenuItem key={item.PCD_ID} value={item.PCD_ID}>
                        {item.PCD_NOME}
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
                {formik.touched.ALU_PCD && formik.errors.ALU_PCD ? (
                  <ErrorText>{formik.errors.ALU_PCD}</ErrorText>
                ) : null}
              </div>
              <InputGroup2>
      ...
      <ModalConfirmacao
        ...
        onHide={() => {
          setModalShowConfirm(false); modalStatus && Router.push(`/municipio/${query.id}/escola/${query.escId}/alunos`);
        }}
        text={
          modalStatus
      ...
      <ModalAviso
        ...
        onConfirm={() => {
          Router.push(`/municipio/${query.id}/escola/${query.escId}/alunos`);
        }}
