...
import { getTestFile, getTestCard } from "src/services/testes.service";
...
export function TableTesteAvailable({ id, escId, url }) {
  ...
  async function loadTestes(
    ...
    let respTestes = null;
    respTestes = await getAssessmentsDownload(
      search,
      page,
      limit,
      selectedColumn,
    ...
    respTestes.data.items?.map((x) => {
      list.push(
        createData(
          ...
          x.edition,
          x.subject,
          x.availableAt && format(new Date(x.availableAt), "dd/MM/yyyy"),
          x.startAt &&
            `${format(new Date(x.startAt), "dd/MM/yyyy")} - ${format(
              new Date(x.endsAt),
              "dd/MM/yyyy"
            )}`,
          x.serie,
  ...
  const generateCard = async (idTeste: number, teste) => {
    try {
      setLoading(true);
      const resp = await getTestCard(idTeste, id, escId);
  ...
  const setRow = (row, index) => {
    ...
    return (
      <TableRowStyled role="checkbox" tabIndex={-1} key={row.id}>
        ...
        <TableCellBorder>{row.disciplina}</TableCellBorder>
        <TableCellBorder>{row.dataDisponivel}</TableCellBorder>
        <TableCellBorder>{row.periodo}</TableCellBorder>
        <TableCellBorder>{row.serie}</TableCellBorder>
        <TableCellBorder
          style={{
            maxWidth: 210,
            overflow: "hidden",
