...
import { editTest } from "src/services/testes.service";
...
export default function FormEditTeste({ teste }) {
  ...
  const { data: dataYears } = useGetYears(null, 1, 999999, null, 'ASC', true);
  ...
  async function changeTeste() {
    ...
    const response = await editTest(teste.TES_ID, teste, null, null);
    if (response.status === 200 && response.data.TES_NOME === teste.TES_NOME) {
      setActive(teste.TES_ATIVO);
    ...
    <>
      ...
      <ModalConfirmacao
        ...
        onHide={() => {
          ...
          Router.push(`/teste/editar/${teste.TES_ID}`);
        }}
