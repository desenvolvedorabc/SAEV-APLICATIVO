...
export default function TableTeste(props) {
  ...
  async function loadTestes(search: string, page: number, limit: number, selectedColumn: string, order: string) {
    const respTestes = await getTests(search, page, limit, selectedColumn, order.toUpperCase())
  ...
  const createExport = async (search: string) => {
    const respTestes = await getTests(search, 1, 9999, null, "ASC")
  ...
  const handleChangeSearch = (e) => {
    ...
    setPage(1)
    loadTestes(e.target.value, 1, limit, selectedColumn, order)
  }
