...
import { createRegionalUnica, editRegionalUnica } from "src/services/regionais-estaduais.service";
...
export default function FormEditRegionalUnica({ regional }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try{
        response = regional
          ? await editRegionalUnica(regional?.id, newValues)
          : await createRegionalUnica(values);
      }
      catch (err) {
        setIsDisabled(false)
      } finally {
  ...
  const getStateDisabled = () => {
    if(user?.USU_SPE?.role === 'MUNICIPIO_ESTADUAL' || user?.USU_SPE?.role === 'ESTADO' || regional){
      return true
    }
    return false
  } 

  const getCountyDisabled = () => {
    if(user?.USU_SPE?.role === 'MUNICIPIO_ESTADUAL' || regional || !state){
      return true
    }
    return false
  } 
      ...
      <Card>
        ...
        <Form onSubmit={formik.handleSubmit}>
          <InputGroup3>
            ...
            <div>
              <AutoCompletePagMun2 county={county} changeCounty={handleChangeCounty} stateId={state?.id} disabled={getCountyDisabled()} />
            </div>
          </InputGroup3>
