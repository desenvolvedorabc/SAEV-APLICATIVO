...
function createData(id: string, name: string, county: string, schools: string): Data {
  return {
    id,
    name,
    county,
...
export default function TableRegionaisUnicas() {
  ...
  const { data, isLoading } = useGetRegionais(debouncedSearchTerm, page, limit, null, order?.toUpperCase(), filterCounty, filterState, 'UNICA');
  ...
  const downloadCsv = async () => {
    ...
    if(!resp.data.message) {
      saveAs(resp?.data, 'Regional_Unica.csv');
    } else {
    }
    setIsDisabled(false)
  };
    ...
    <Container>
      ...
      {showFilter && (
        <FilterSelectedContainer>
          ...
          <div className="me-2">
            <AutoCompletePagMun2 county={selectedCounty} changeCounty={handleSelectCounty} disabled={!selectedState || user?.USU_SPE?.role === 'MUNICIPIO_ESTADUAL'} stateId={selectedState?.id} width={"150px"} />
          </div>
          <div>
            <ButtonWhite
              onClick={() => {
              ...
              <TableBody>
                {rows.length > 0 ? (
                  rows.map((row, index) => {
                    ...
                    return (
                      <Link href={`/regional-unica/editar/${row.id}`} key={row.id} passHref>
