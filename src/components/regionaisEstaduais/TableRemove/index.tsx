...
export default function TableRemove({ listAdded, removeItem }: Params) {
  ...
  useEffect(() => {
    let listFiltered = listAdded?.sort((a, b) => a.MUN_NOME.localeCompare(b.MUN_NOME))
    if (search)
      listFiltered = listFiltered.filter((city) =>
        city.MUN_NOME
          .toUpperCase()
          .normalize('NFD')
          .replace(/[\u0300-\u036f]/g, '')
    ...
    setListCityFilter(paginatedList)
  ...
  return (
    <div style={{ width: '100%', padding: '20px' }}>
      <div style={{ marginBottom: 24 }}>Municípios Adicionados</div>
      <div>
        <TextField
          id="city"
          label="Pesquisar Município Adicionado"
          size="small"
          fullWidth
          value={search}
      ...
      >
        <Table stickyHeader aria-label="simple table">
          <TableHead>
            <TableRow>
              <TableCellStyled><strong>MUNICÍPIOS ADICIONADOS ({listAdded?.length})</strong></TableCellStyled>
          ...
          <TableBody>
            {listCityFilter.length > 0 ? (
                listCityFilter.map((city) => (
                  <TableRow key={'cityRemove' + city.MUN_ID}>
                ...
                <div style={{ padding: 10 }}>
                  Nenhum município foi adicionado.
                </div>
            )}
          </TableBody>
        </Table>
