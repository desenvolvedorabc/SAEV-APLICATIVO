...
function createData(MUN_ID: string, MUN_NOME: string, MUN_COD_IBGE: string, TOTAL_ESCOLAS: number, TOTAL_ALUNOS: number, ENTURMADOS: number, MUN_STATUS: string): Data {
  return {
    MUN_ID,
    MUN_NOME,
    MUN_COD_IBGE,
...
export default function TableMunicipios() {
  ...
  const [orderBy, setOrderBy] = useState("MUN_NOME");
  ...
  const { data, isLoading } = useGetCountiesReport(search, page, limit, selectedColumn, order, filterStatus);
                      ...
                      <Link href={`/municipio/${row.MUN_ID}`} key={row.MUN_ID} passHref>
                        <TableRowStyled role="checkbox" tabIndex={-1}>
                          ...
                          <TableCellBorder align="center">
                            <Circle color={statusColor(row.MUN_STATUS)} />
                          </TableCellBorder>
                        </TableRowStyled>
                      </Link>
                    );
